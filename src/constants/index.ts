...
export const booksSearch = (
  ...
  `${base_url}${query}${parameter}${term}${filter}${order}&maxResults=36&key=${import.meta.env.VITE_GOOGLE_BOOKS_API_KEY}`;
export const bookDetails = (book_id: any) => `${base_url}${book_id}?key=${import.meta.env.VITE_GOOGLE_BOOKS_API_KEY}`;